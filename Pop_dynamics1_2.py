# -*- coding: utf-8 -*-
"""
Created on Mon Jan  4 13:39:04 2021

UPDATE: The dynamics are now callable as a function so they may be interfaced
with a spectroscopic simulator
...
def FC_overlap(disp,a_max,b_max):
    ...
    for a in range(1,nu_max+1):
        FC[0][a]=-disp/np.sqrt(a*1.0)*FC[0][a-1]
        FC[a][0]=((-1.0)**a)*FC[0][a]
        
    #remaining elements
    for a in range(1, nu_max+1):
        for b in range(a, nu_max+1):
            FC[a][b]=-disp/np.sqrt(1.0*b)*FC[a][b-1]+np.sqrt((1.0*a)/(1.0*b))*FC[a-1][b-1]
...
def dndt(n_ia,t,n_ia_shape,w_i,w_mode,k_alpha,FC,k_IC,pump_param):
    ...
    ************************************************************************'''
    ...
    Currently this characterizes excitation by a 'sech squared' laser pulse of 
    finite width. It is assumed that the pusle is sufficiently narrow
    ...
    ************************************************************************'''
    ...
    for element in mode_iter: #Interate through the vibronic levels on the ground state
        ...
        for element2 in mode_iter2:
            ...
            #The FWHM duration is pump_param[5]
            #the tau parameters in the sech^2 pulse is ~1/1.76 of this value
            tau=pump_param[5]/1.76
            Gamma=(1.0/(2.0*tau))\
            *(np.square(1.0/np.cosh((t-pump_param[4])/tau)))
            
            #Depopulate the ground state
            dndt_pump[0][a]-=pump_param[1]*FC_square*gauss*Gamma*n_ia[0][a]
            
            #populate the excited state
            dndt_pump[pump_param[0]][b]+=pump_param[1]*FC_square*gauss*Gamma*n_ia[0][a]
...
"""
(8) The actual evolution of the system. This function receives all required variables
    externally, sets up the system and carries out the numerical solution of the dynamics
...
Input: (i) n_ia = a flattend array of current/initial population values
       ...
       (ix) pump_param = Tuple of parameters for the pump. In order this contains
            ...
            (e) t_pump0=arrival time of the pulse
            (f) t_pump_duration=pump duration
       (x) ts = a tuple of the timesteps one wishes to cover. 

Output: (i) dndt = a flattend array of time derivatives of the vibronic populations. 
 
"""    

def evol(beta,swi_thermal,N_elec,wi,L_IC_ij,G_IC_ij,N_mode,N_vib_level,w_mode,L_IVR_i,G_IVR_i,disp,pump_param,ts):
    ...
    ****************************************************************************"""
    ...
    #this will have N_mode+1 indices
    ...
    if swi_thermal=='Y' or swi_thermal=='y': 
    ...
    #asolute ground state (default)?
    else:
        abs_gs_index=np.zeros(N_mode+1,dtype=int) #establishing the indices of the absolute gs
        abs_gs_index=tuple(abs_gs_index)
        n_init[abs_gs_index]=1.0
